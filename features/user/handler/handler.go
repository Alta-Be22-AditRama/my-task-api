...
func (uh *UserHandler) Update(c echo.Context) error {
	...
	// memanggil/mengirimkan data ke method service layer
	err := uh.userService.Update(uint(idConv), inputCore)
	if err != nil {
		// validasi code response
		if strings.Contains(err.Error(), "validation") {
			return c.JSON(http.StatusBadRequest, responses.JSONWebResponse("error update data: "+err.Error(), nil))
		}
		return c.JSON(http.StatusInternalServerError, responses.JSONWebResponse("error update data: "+err.Error(), nil))
	}
	return c.JSON(http.StatusOK, responses.JSONWebResponse("success update user", nil))
}
